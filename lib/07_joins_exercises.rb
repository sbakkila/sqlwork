...
def andrews_films_and_leads
  ...
  JOIN
    ...
    JOIN
      actors ON castings.actor_id = actors.id
    WHERE
      actors.name = 'Julie Andrews') as julie_films
      ON movies.id = julie_films.id
    WHERE
      castings.ord = 1
  SQL
end
...
def films_by_cast_size
  ...
  execute(<<-SQL)
  SELECT
    title, COUNT(actors.name) AS num_actors
  FROM
    movies
  JOIN
    castings ON movies.id = castings.movie_id
...
def colleagues_of_garfunkel
  ...
  JOIN
    actors ON castings.actor_id = actors.id
  WHERE
    actors.name = 'Art Garfunkel') as garfunkel_movies
  ON movies.id = garfunkel_movies.id
